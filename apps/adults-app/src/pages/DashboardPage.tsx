...
export const DashboardPage: React.FC = () => {
  ...
  // Seed mock streams and recordings data to Firestore so the site looks highly populated
  useEffect(() => {
    if (!user) return;
    ...
    const seedMockData = async () => {
      ...
      try {
        for (const mockStr of mockStreams) {
          const { id, ...data } = mockStr;
          await setDoc(doc(db, 'streams', id), data);
          await setDoc(doc(db, 'streams_kids', id), data);
        }
        for (const mockRec of mockRecordings) {
          const { id, ...data } = mockRec;
          await setDoc(doc(db, 'recordings', id), data);
        }
        console.log("Mock streams and recordings seeded successfully!");
      } catch (err) {
